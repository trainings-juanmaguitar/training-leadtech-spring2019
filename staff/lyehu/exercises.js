//Hacer este ejercicio → _Write a function charFreq()
//that takes a string and builds a frequency listing of the characters contained in it.
...
const getWordsFreq = aWords => {
  return aWords.reduce((oWordsFreq, word) => {
    oWordsFreq[word] ? (oWordsFreq[word] += 1) : (oWordsFreq[word] = 1);
    return oWordsFreq;
  }, {});
};

const getMostFrequentWords = text => {
  const aWords = parseText(text)
    .toLowerCase()
    .split(" ");
  const oWordsFreq = getWordsFreq(aWords);
  return Object.values(oWordsFreq)
    .sort((a, b) => {
      return b - a;
    })
    .splice(0, 10)
    .map(frequency =>
      Object.keys(oWordsFreq).find(key => oWordsFreq[key] === frequency)
    );
};

console.log(getMostFrequentWords(text));
